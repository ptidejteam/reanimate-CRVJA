...
describe("parseBankFile", () => {
  ...
  test("fails when file is truncated", async () => {
    const data = new Uint8Array([
      0x41, 0x6d, 0x53, 0x70, // AmSp
      0x00, 0x01,             // 1 sprite expected
      0x00, 0x01,             // width=1
      0x00, 0x08,             // height=8
      0x00, 0x04              // depth=4. Missing hotspot and graphic data.
    ]);
    const file = new File([data], "truncated.abk");
    await expect(parseBankFile(file)).rejects.toThrow("Unexpected end of file while reading header");
  });
  ...
  test("fails when dimensions are invalid or excessive", async () => {
    const data = new Uint8Array([
      ...
      0x00, 0x08,             // height = 8
      0x00, 0x04,             // depth = 4
      0x00, 0x00,             // hotspotX = 0
      0x00, 0x00              // hotspotY = 0
    ]);
